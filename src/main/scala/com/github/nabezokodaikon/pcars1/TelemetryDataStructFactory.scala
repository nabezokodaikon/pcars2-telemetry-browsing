  ...
  def createTelemetryData(data: List[Byte]): TelemetryData = {
    ...
    TelemetryData(
      ...
      carStateVecotrDataX = CarStateVecotrData(
        ...
        worldVelocity = worldVelocity(0),
        angularVelocity = angularVelocity(0),
        localAcceleration = localAcceleration(0),
        worldAcceleration = worldAcceleration(0),
        extentsCentre = extentsCentre(0)),
      carStateVecotrDataY = CarStateVecotrData(
        orientation = orientation(1),
        localVelocity = localVelocity(1),
        worldVelocity = worldVelocity(1),
        angularVelocity = angularVelocity(1),
        localAcceleration = localAcceleration(1),
        worldAcceleration = worldAcceleration(1),
        extentsCentre = extentsCentre(1)),
      carStateVecotrDataZ = CarStateVecotrData(
        orientation = orientation(2),
        localVelocity = localVelocity(2),
        worldVelocity = worldVelocity(2),
        angularVelocity = angularVelocity(2),
        localAcceleration = localAcceleration(2),
        worldAcceleration = worldAcceleration(2),
        extentsCentre = extentsCentre(2)),
      tyresDataFrontLeft = TyresData(
        terrain = terrain(0),
        tyreY = tyreY(0),
        tyreRPS = tyreRPS(0),
        tyreSlipSpeed = tyreSlipSpeed(0),
        tyreTemp = tyreTemp(0) / 255f,
        tyreGrip = tyreGrip(0) / 255f,
        tyreHeightAboveGround = tyreHeightAboveGround(0),
        tyreLateralStiffness = tyreLateralStiffness(0),
        tyreWear = tyreWear(0) / 255f,
        brakeDamage = brakeDamage(0) / 255f,
        suspensionDamage = suspensionDamage(0) / 255f,
        brakeTempCelsius = brakeTempCelsius(0) / 1f,
        tyreTreadTemp = tyreTreadTemp(0) / 1f,
        tyreLayerTemp = tyreLayerTemp(0) / 1f,
        tyreCarcassTemp = tyreCarcassTemp(0) / 1f,
        tyreRimTemp = tyreRimTemp(0) / 1f,
        tyreInternalAirTemp = tyreInternalAirTemp(0) / 1f,
        wheelLocalPositionY = wheelLocalPositionY(0),
        rideHeight = rideHeight(0),
        suspensionTravel = suspensionTravel(0),
        suspensionVelocity = suspensionVelocity(0),
        airPressure = airPressure(0) / 1f),
